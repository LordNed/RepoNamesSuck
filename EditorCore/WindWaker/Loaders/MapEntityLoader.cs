    ...
    public class MapEntityLoader
    {
        ...
        public List<MapEntityData> LoadFromStream(EndianBinaryReader reader)
        {
            ...
            for (int i = 0; i < chunks.Count; i++)
            {
                ...
                if (template == null)
                {
                    WLog.Warning(LogCategory.EntityLoading, null, "Unsupported entity format {0}. Map will save without this data!", chunk.FourCC);
                    continue;
                }
        ...
        private object ResolveEntityReferenceNew(string askingChunkFourCC, ItemJsonTemplate.Property templateProperty, int index, List<MapEntityData> loadedEntities)
        {
            switch (templateProperty.ReferenceType)
            {
                case "Room":
                    WLog.Error(LogCategory.EntityLoading, null, "Chunk {0} requesting Room reference, not-implemented. Ignoring.", askingChunkFourCC);
