...
namespace GameFormatReader.Common
{
    ...
    public sealed class EndianBinaryReader : BinaryReader
    {
        ...
        private byte m_bitPool;
        private byte m_bitPoolSize;

        public uint ReadBits(int paramSize)
        {
            uint output = 0;
            uint numBitsSoFar = 0;

            while(numBitsSoFar < paramSize)
            {
                uint numBitsLeft = (uint)paramSize-numBitsSoFar;
                if (m_bitPoolSize == 0)
                {
                    m_bitPool = ReadByte();
                    m_bitPoolSize = 8;
                ...
                byte numBits;
                if(numBitsLeft >= m_bitPoolSize)
                    numBits = m_bitPoolSize;
                else
        ...
        /// <summary>
        /// Reads characters from the underlying <see cref="Stream"/>
        /// until numChars is reached.
        /// </summary>
        /// <param name="numChars">The number of characters to attempt to read.</param>
        /// <returns>The string of characters read until numChars was hit.</returns>
